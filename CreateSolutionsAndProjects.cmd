...
rem create the class library for the Data Access Layer and add it to the
solution
...
dotnet new classlib -n SpyStore.Models -o .\SpyStore.Models -f
netcoreapp2.1
dotnet sln SpyStore.sln add SpyStore.Models
rem create the XUnit project for the Data Access Layer and add it to the
solution
dotnet new xunit -n SpyStore.Dal.Tests -o .\SpyStore.Dal.Tests -f
netcoreapp2.1
dotnet sln SpyStore.sln add SpyStore.Dal.Tests
rem create the XUnit project for the Service and add it to the solution
dotnet new xunit -n SpyStore.Service.Tests -o .\SpyStore.Service.Tests -f
netcoreapp2.1
dotnet sln SpyStore.sln add SpyStore.Service.Tests
rem create the ASP.NET Core RESTful service project and add it to the
solution
dotnet new webapi -n SpyStore.Service -au none --no-httpsꂠ-o .\SpyStore. Service -f netcoreapp2.1
dotnet sln SpyStore.sln add SpyStore.Service
rem create the ASP.NET Core Web Application project and add it to the
solution
dotnet new mvc -n SpyStore.Mvc -au none --no-httpsꂠ-o .\SpyStore.Mvc -f
netcoreapp2.1
